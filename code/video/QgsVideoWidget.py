...
class VideoWidget(QVideoWidget):
    ...
    def mouseMoveEvent(self, event, useLast=False):
        ...
        if (
            self._interaction.pointDrawer
            or self._interaction.polygonDrawer
            or self._interaction.lineDrawer
            or self._interaction.measureDistance
            or self._interaction.measureArea
            or self._interaction.censure
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if self._interaction.objectTracking:
                self.Tracking_Video_RubberBand.setGeometry(QRect(self.origin, QSize()))
                self.Tracking_Video_RubberBand.show()

            # Censure Interaction
            if self._interaction.censure:
                self.Censure_RubberBand.setGeometry(QRect(self.origin, QSize()))
